...
class Base_Model extends CI_Model 
{
    ...
    public function getUserDetails($user_id, $select_arr ='*',$user_name='') 
    {
...
// $this->db->where('customer_name',$customer_id);
  ...
  //       die();
    ...
    public function getVehicleAuto($term='') {

        $output = [];
        $this->db->select('id,vehicle_name,vehicle_number');
        $this->db->from('vehicles');
        if ($term) {
            $this->db->group_start();  
            $this->db->like('vehicle_number', $term);
            $this->db->or_like('vehicle_name', $term);
            $this->db->group_end(); 
        }
        $this->db->limit(10);
        ...
        foreach($res->result_array() as $row) {
            ...
            'text' =>$row['vehicle_number']. '-' .$row['vehicle_name']  
        ];
    }
...
public function getBarcodeAuto($term='') {
...
public function getVehicleName($id) 
{
    ...
    $vehicle_name = NULL;
    $this->db->select('vehicle_name,vehicle_number');
    $this->db->from('vehicles');
    $this->db->where('id', $id);
    $this->db->limit(1);
    $query = $this->db->get();
    foreach ($query->result_array() as $row) {
        $vehicle_name = $row['vehicle_number'] . '-' . $row['vehicle_name'];
    }
...
public function getAssignedItemAuto($term = '') {
    ...
    $this->db->where('jd.salesman_id', log_user_id());
...
public function getAssignedCategoryAuto($term = '') {
    ...
    $this->db->from('job_details jd');
    $this->db->join('category c', 'FIND_IN_SET(c.id, jd.categories) > 0', 'inner');
    $this->db->where('jd.salesman_id', log_user_id());
    ...
    foreach ($res->result_array() as $row) {
        $output[] = ['id' => $row['id'], 'text' => $row['category_name']];
    }
