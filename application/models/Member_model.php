...
class Member_model extends Base_model {
    ...
    public function updateUserProfile($user_details, $user_id) {
        ...
        if(array_key_exists('address', $user_details)){
         $user_details['address'] = htmlentities($user_details['address']);
...
{
    $this->db->select('*');
    $this->db->from('pending_sales');
    $this->db->where('status','pending');

    $count = $this->db->count_all_results();
    return $count;
}
public function getAllPendingSalesAjax($search_arr = [], $count = 0) {
    $row = $search_arr['start'];
    $this->load->model('Jobs_model');
    $rowperpage = $search_arr['length'];

    $this->db->select('ps.*');
    $this->db->select('jd.quantities,jd.products,jd.categories');
    $this->db->from('pending_sales as ps');
    $this->db->join('job_details as jd','jd.id=ps.ref_id');
    ...
    foreach ($query->result_array() as $row) {
        $row['index'] = $search_arr['start'] + $i;
        $product_details = $this->Jobs_model->getSalesProductDetails($row['products']);
        ...
        $row['salesman'] = $this->getUserName($row['salesman_id']);
...
public function getAllPendingSales($id='',$status='all') {
...
public function updatePendingSalesStatus($id,$status)
{
    $this->db->set('status',$status)
    ->set('done_by',log_user_id())
    ->where('id',$id);
    $update = $this->db->update('pending_sales');
    return $update;
}
public function updateSalesCount($field_count, $value,$id)
{
 
 
    $this->db->set($field_count, $field_count .'+ '. $value, false);
