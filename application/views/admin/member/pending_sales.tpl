	...
	<script type="text/javascript">
		...
		$(document).ready(function(){ 
			...
			var order = $('#customer_list').DataTable({
				...
				'ajax': {
					'url':'{base_url()}{log_user_type()}/member/get_pending_sales_ajax',
					"type": "POST", 
					"data" : {
				...
				{ data: 'id', mRender: function(data, type, row) {
					var  link= ''
					link +="<a   title='Approve' data-encrypted_id= '"+row.enc_item_id+"' class='btn btn-primary btn-round approve_sales'><i class='fa fa-check'></i></a>";

					link +="<a data-encrypted_id= '"+row.enc_item_id+"' class='btn btn-danger btn-round reject_sales' data-placement='top' data-original-title='delete'><i class='fa fa-trash'></i></a>";
					return link;
				}}, 
				],
		...
		$(document).on('click', '.approve_sales', function(event) {
			event.preventDefault();
			var id = $(this).data('encrypted_id');
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = "{base_url()}{log_user_type()}/member/approve_sales/approve/"+id; 
				} else {
					swal('Cancelled', 'Data Safe', "error");
				}
			});
		});
		$(document).on('click', '.reject_sales', function(event) {
			event.preventDefault();
			var id = $(this).data('encrypted_id');
			...
			function (isConfirm) {
				if (isConfirm) {
					document.location.href = "{base_url()}{log_user_type()}/member/approve_sales/reject/"+id; 
				} else {
					swal('Cancelled', 'Data Safe', "error");
				}
			});
