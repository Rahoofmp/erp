...
class Salesman extends Base_Controller {
	...
	{   	
		...
		if ($this->input->post('add_salesman') && $this->validate_add_salesman()) {
			...
			$ins=$this->Salesman_model->addsalesmanDetails($post_arr);

			if($ins){
				$this->Base_model->insertIntoActivityHistory(log_user_id(), log_user_id(),'salesman Registered', serialize($post_arr));
				...
				$msg = 'Registration completed Successfully'.' , ID : '.$post_arr['salesman_username'].' ,password : '.$post_arr['password'];
