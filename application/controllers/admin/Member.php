...
class Member extends Base_Controller {
	...
	{ 
		...
		if( $this->input->post('submit') )
		{
			if( $this->input->post('submit') == 'reset')
			{
				...
				// $search_arr['staff_id'] = $post_arr['staff_id'];
	...
	{ 
		...
		if( $this->input->post( 'profile_update' ) )
		{
			...
			if(array_key_exists('status', $post_arr)){
				$this->Base_model->changeStatus($user_id,$post_arr['status']);

				$this->Base_model->insertIntoActivityHistory($user_id,log_user_id(),'status change');
			}


			
			if (log_user_type()=='admin') {
				$post_arr['department']='admin';
			}
		...
		$data['user_status']=$this->Base_model->getUserStatus($user_id);
